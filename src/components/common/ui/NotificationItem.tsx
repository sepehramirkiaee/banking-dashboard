...
function NotificationItem({ notification, removeNotification }: { notification: Notification; removeNotification: (id: string) => void }) {
  ...
  return (
    <div
      className={`relative border border-gray-100 flex items-center gap-3 p-4 bg-white rounded-md shadow-lg transition-all duration-500 ${visible ? "opacity-100" : "opacity-0"}`}
